...
def exec(Connection connection, Map input) {
    ...
    List railwayGeom = [[0.0, 0.0, 0.0], [1000.0, 0.0, 0.0]]
    ...
    List<Double> lastPoint = railwayGeom[0] as List<Double>
    ...
    for (int i = 0; i < railwayGeom.size() - 1; i++) {
        List<Double> start = railwayGeom[i]
        List<Double>  end = railwayGeom[i + 1]
        ...
        if (i > 0) {
            Double dxBuffer = start[0] - lastPoint[0]
            Double dyBuffer = start[1] - lastPoint[1]
            Double dzBuffer = start[2] - lastPoint[2]
            ...
            Double x1 = start[0] + dx * ratio
            Double y1 = start[1] + dy * ratio
            Double z1 = start[2] + dz * ratio
...
// Creeate final GeoJSON file
    ...
    def folderUse = "/src/test/resources/org/noise_planet/noisemodelling/scripts/Dynamic/Rail/TrainExport/"
