	...
	public Value evaluate(final Value... args) throws FunctionException {
		if (args[0].isNull()) {
			...
			if(points.length==0)
				return ValueFactory.createNullValue();
			if(points.length==1)
				return ValueFactory.createValue(gf.createPoint(points[0]));
			...
			{
				if(points.length==2)
					return ValueFactory.createValue(gf.createPoint(new Coordinate((points[1].x+points[0].x)/2.,
							(points[1].y+points[0].y)/2.,
							(points[1].y+points[0].y)/2.)));
				else
					return ValueFactory.createValue(Line.getInteriorPoint());
			}
			...
			Point[] deltaPoints=new Point[nbpts];
			int pts=0;
			...
			while(pts<nbpts)
			{
				...
				refpt=nextbldpts;
				deltaPoints[pts]=gf.createPoint(nextbldpts);
				pts++;
				distToNextPt=ModifiedDelta;				
			}
			return ValueFactory.createValue(gf.createMultiPoint(deltaPoints));
		}
	}
