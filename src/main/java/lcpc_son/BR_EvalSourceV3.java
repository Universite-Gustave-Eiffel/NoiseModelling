...
public class BR_EvalSourceV3 implements Function {
	...
	public Value evaluate(Value... args) throws FunctionException {
		if (args.length<9) {
			...
			// Compute the slope
			// final double ground_dist=Math.sqrt(Math.pow(road_length,2)-Math.pow(end_z-begin_z,2));
			// OrbisGis return the length without the Z data; then we don't need to compute the zero level distance
	...
	public Arguments[] getFunctionArguments() {
		return new Arguments[] { new Arguments(
				...
				Argument.NUMERIC		//Road length(m) (without taking account of the Z coordinate)
		)}; 	
	}
	...
	public String getSqlOrder() {
		return "select BR_EvalSourceV3(loadSpeed,junction,speedMax,lightVehicleCount,heavyVehicleCount,roadType,Zbegin,Zend,roadLength) from myTable;";
	}
