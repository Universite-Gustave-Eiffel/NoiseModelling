...
public class ThreadPool extends ThreadPoolExecutor {
    ...
    /** Default constructor.
     * Set CorePoolSize size to 32
     * Set Maximum pool size to 256
     * Set Keep Alive Time to 60 seconds
     */
    public ThreadPool()
    {
    ...
    public void executeBlocking(Runnable command) {
		while(this.getQueue().size()+this.getActiveCount()>=this.getMaximumPoolSize())
		{
			try {
				Thread.sleep(100);
