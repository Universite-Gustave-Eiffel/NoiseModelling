...
create table route as select ST_LineFromText('LINESTRING(100 100 0, 101 100 0)') the_geom, 50 VIT_VL, 50 VIT_PL, 300 MT, 30 PT;
...
--CREATE TABLE route_srcf AS SELECT id, the_geom,db_m,
...
create table test_LEQ  as SELECT * from BR_PTGRID3D('batiments', 'HAUTEUR',  'route_srcf', 'recepteur', 'DB_M','','', 750, 50, 0, 0, 0);

drop table if exists test_LEQ_ATT ;
create table test_LEQ_ATT as  SELECT * from BR_PTGRID3D_ATT_F('batiments', 'HAUTEUR', 'route_src_pt_100', 'recepteur', 'DB_M','', '', 750, 50, 0, 0, 0);