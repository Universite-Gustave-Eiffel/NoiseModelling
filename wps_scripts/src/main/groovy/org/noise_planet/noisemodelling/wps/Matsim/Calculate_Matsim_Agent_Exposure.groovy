...
def exec(Connection connection, input) {
    ...
    for (Map.Entry<Id<Person>, Person> entry : persons.entrySet()) {
        ...
        XYSeries homeLaeqSeries = new XYSeries("Home_LAeq");
        XYSeries doseSeries = new XYSeries("Dose");
        XYSeries homeDoseSeries = new XYSeries("Home_Dose");
        ...
        for (int slice = 0; slice < nbSlices; slice++) {
            ...
            for (PlanElement element : plan.getPlanElements()) {
                ...
                while(result.next()) {
                    ...
                    laeqSeries.add((slice * secondsInSlice * 1000), result.getDouble("LEQA"));
                    if (homeId == activityId) {
                        homeLAeq = 10 * Math.log10(Math.pow(10, homeLAeq / 10) + timeWeight * Math.pow(10, result.getDouble("LEQA") / 10));
                        homeLaeqSeries.add(slice * secondsInSlice * 1000, result.getDouble("LEQA"));
                        hasHomeLevel = true;
                    }
                    hasLevel = true;
                }
                if (homeId != activityId) {
                    ...
                    while (homeResult.next()) {
                        homeLAeq = 10 * Math.log10(Math.pow(10, homeLAeq / 10) + timeWeight * Math.pow(10, homeResult.getDouble("LEQA") / 10));
                        homeLaeqSeries.add(slice * secondsInSlice * 1000, homeResult.getDouble("LEQA"));
                        hasHomeLevel = true;
                    }
                }
            }

            doseSeries.add((slice * secondsInSlice * 1000), LAeq);
            homeDoseSeries.add((slice * secondsInSlice * 1000), homeLAeq);

            if (!hasLevel) {
                laeqSeries.add((slice * secondsInSlice * 1000), null);
            }
            if (!hasHomeLevel) {
                homeLaeqSeries.add((slice * secondsInSlice * 1000), null);
            }
        }
        ...
        if (agentId > 0) {
            ...
            JFreeChart chart = ChartFactory.createXYStepChart("AgentId : " + agentId, "Time", "Noise Level", dataset);
            ...
            for (PlanElement element : plan.getPlanElements()) {
                if (element instanceof Activity) {
                    ...
                    XYTextAnnotation textAnnotation = new XYTextAnnotation(activity.getType(), textX, yAnnotationPos + 1);
                    ...
                    if (leg.getDepartureTime().isDefined()) {
                        legStart = leg.getDepartureTime().seconds();
                    }
                    double legEnd = 86400;
                    if (leg.getTravelTime().isDefined()) {
                        legEnd = legStart + leg.getTravelTime().seconds();
                    }
