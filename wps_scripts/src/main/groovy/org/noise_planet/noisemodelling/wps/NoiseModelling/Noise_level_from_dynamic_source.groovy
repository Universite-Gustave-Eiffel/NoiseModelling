...
title = 'Calculation of the L(t) map from a dynamic noise emission table'
description = 'Calculation of the L(t) map from the road noise emission table. </br> Tables must be projected in a metric coordinate system (SRID). Use "Change_SRID" WPS Block if needed. ' +
        '</br> </br> <b> The output table is called : LT_GEOM </b> ' +
        'and contain : </br>' +
        '-  <b> IDRECEIVER  </b> : an identifier (INTEGER, PRIMARY KEY). </br>' +
        '-  <b> T  </b> : the timestep (INTEGER). </br>' +
...
inputs = [
        ...
        tableSources            : [
                ...
                description: '<b>Name of the Sources table.</b></br>  ' +
                        ...
                        '- <b> LW63, LW125, LW250, LW500, LW1000, LW2000, LW4000, LW8000 </b> : 8 columns giving the day emission sound level for each octave band (FLOAT). </br> ' +
                        '- <b> T </b> : 8 columns giving the evening emission sound level for each octave band (FLOAT).</br> ' +
...
def forgeCreateTable(Sql sql, String tableName, LTConfig ltConfig, String geomField, String tableReceiver, String tableResult) {
    ...
    List<Integer> freqLvl = ltConfig.getPropagationProcessPathData(LTConfig.TIME_PERIOD).freq_lvl;
...
def exec(Connection connection, input) {
    ...
    logger.info('Start : LT from Emission')
    ...
    LConfig ltConfig = new LConfig(LTConfig.INPUT_MODE.INPUT_MODE_LWT)
    ltConfig.setComputeLT(true)
    ltConfig.setMergeSources(!confExportSourceId)
    ...
    if (input['confRaysName'] && !((input['confRaysName'] as String).isEmpty())) {
        ...
        if(confRaysName.startsWith("file:")) {
            ...
            ltConfig.setExportRaysMethod(LTConfig.ExportRaysMethods.TO_RAYS_TABLE)
            ltConfig.setRaysTable(input['confRaysName'] as String)
        }
        ltConfig.setKeepAbsorption(true);
        ltConfig.setMaximumRaysOutputCount(maximumRaysToExport);
    }

    LTPointNoiseMapFactory ltProcessing = new LTPointNoiseMapFactory(connection, ltConfig)

    // add optional discrete directivity table name
    if(tableSourceDirectivity.isEmpty()) {
        // Add train directivity
        ltProcessing.insertTrainDirectivity()
    } else {
        // Load table into specialized class
        ltProcessing.directionAttributes = DirectivityTableLoader.loadTable(connection, tableSourceDirectivity, 1)
        logger.info(String.format(Locale.ROOT, "Loaded %d directivity from %s table", ltProcessing.directionAttributes.size(), tableSourceDirectivity))
    }
    ...
    pointNoiseMap.setPropagationProcessPathData(LTConfig.TIME_PERIOD.DAY, environmentalDataDay)
    pointNoiseMap.setPropagationProcessPathData(LTConfig.TIME_PERIOD.EVENING, environmentalDataEvening)
    pointNoiseMap.setPropagationProcessPathData(LTConfig.TIME_PERIOD.NIGHT, environmentalDataNight)
    ...
    pointNoiseMap.setComputeRaysOutFactory(ltProcessing)
    pointNoiseMap.setPropagationProcessDataFactory(ltProcessing)
    ...
    profilerThread.addMetric(ltProcessing);
    ...
    // Associate Geometry column to the table LT
    StringBuilder createdTables = new StringBuilder()

    sql.execute("drop table if exists LT_GEOM;")
    logger.info('create table LT_GEOM')
    forgeCreateTable(sql, "LT_GEOM", ltConfig, geomFieldsRcv.get(0), receivers_table_name,
            ltConfig.lTTable)
    createdTables.append(" LT_GEOM")
    sql.execute("drop table if exists " + TableLocation.parse(ltConfig.getlTTable()))
    ...
    logger.info('End : LT from Emission')
