# Use NoiseModelling with Python

The following sample demonstrate the use of the Python scripting language to command the execution of NoiseMap. The advantage is the ability to run it on dedicated server without X11.

A ready package published here contain all usefull dependencies:
...
Install psycopg2 http://initd.org/psycopg/docs/install.html#installation

Run spatial database by using this command

## Windows
```bat
java -cp bin/*;bundle/*;sys-bundle/* org.h2.tools.Server -pg
```
...
This command will run H2 database in server mode. Python will connect to it using the PostreSQL connection protocol.

The python script is available here:

[noisemap1.py](scripts/noisemap1.py)

Save the following script as noisemap1.py file.
...
Run it with python interpreter. It will save the contouring noise map into a shapefile in the current directory.