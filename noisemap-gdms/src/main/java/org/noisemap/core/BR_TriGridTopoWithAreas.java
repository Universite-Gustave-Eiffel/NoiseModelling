...
import static org.noisemap.core.BR_TriGrid.DbaToW;
import static org.noisemap.core.BR_TriGrid.GetGlobalEnvelope;
import static org.noisemap.core.BR_TriGrid.getCellEnv;
/**
 *
 * @author SU Qi
 */
public class BR_TriGridTopoWithAreas extends BR_TriGrid{
	...
	private long totalDelaunay = 0;
		...
		// Builds geom , sources.the_geom, topo points, soil areas, sources.db_m ,max propa dist , subdiv
		...
		try {
			...
			// Load Sources, Buildings, Topo Points and Soil Areas table drivers
                        if(tables.length!=4){
                            throw new FunctionException("Table lenght must be 4 !");
                        }
			final DataSet sds = tables[tableBuildings];
			final DataSet sdsSources = tables[tableSources];
                        final DataSet sdsTopoPts = tables[tableTopoPts]; 
			...
			int spatialTopoPtsFieldIndex = MetadataUtilities.getSpatialFieldIndex(sdsTopoPts.getMetadata());
	...
	public String getSqlOrder() {
		return "create table result as select * from BR_TriGridTopoWithAreas( buildings_table, sound_sources_table, topo_point, soil_areas, 'source db field name',searchSourceLimit,searchReflectionWallLimit,subdivlevel,roadwith(1.8),densification_receiver(5),max triangle area(300),reflection order(2),diffraction order(1),wall absorption(0.1));";
	}

	@Override
	public String getDescription() {
		return "BR_TriGridTopoWithAreas(buildings(polygons),sources(points),topopoints(points), soilareas(polygons, G) sound lvl field name(string),maximum propagation distance (double meter),maximum wall seeking distance (double meter),subdivision level 4^n cells(int), roads width (meter), densification of receivers near roads (meter), maximum area of triangle, sound reflection order, sound diffraction order, alpha of walls ) Sound propagation from ponctual sound sources to ponctual receivers created by a delaunay triangulation of specified buildings geometry.";
	}
}
